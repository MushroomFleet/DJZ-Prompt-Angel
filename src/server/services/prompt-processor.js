...
// Helper function to get the correct system-prompts path for both dev and production
function getSystemPromptsPath() {
    const isDev = process.env.NODE_ENV === 'development';
    const isElectronApp = !!process.versions.electron;
    
    if (isDev) {
        // Development mode - system-prompts are under assets relative to project root
        return path.join(__dirname, '../../../assets/system-prompts');
    }
    
    // In packaged electron app, try multiple locations in order of likelihood
    const fallbackPaths = [
        // extraResources location (most likely with our config)
        process.resourcesPath ? path.join(process.resourcesPath, 'assets', 'system-prompts') : null,
        
        // Standard electron locations
        path.join(process.cwd(), 'assets', 'system-prompts'),
        path.join(__dirname, '../../../assets/system-prompts'),
        
        // Near executable
        path.join(path.dirname(process.execPath), 'resources', 'assets', 'system-prompts'),
        path.join(path.dirname(process.execPath), 'assets', 'system-prompts'),
        
        // App.asar locations (if app is packaged in asar)
        isElectronApp && process.resourcesPath ? path.join(process.resourcesPath, 'app.asar', 'assets', 'system-prompts') : null,
        
        // Last resort - current working directory
        path.join(process.cwd(), 'resources', 'assets', 'system-prompts')
    ].filter(Boolean); // Remove null entries
...
class PromptProcessor {
    ...
    async loadSystemPromptFromContent(content, filename = null) {
        try {
            if (!content || typeof content !== 'string') {
                throw new Error('Invalid system prompt content');
            }
            
            this.systemPrompt = content;
            this.currentSystemPromptFile = filename;
            
            logger.info('System prompt loaded from content', {
            ...
            logger.error('Failed to load system prompt from content:', error);
            throw error;
        }
    }

    getCurrentSystemPromptFile() {
        return this.currentSystemPromptFile || 'VEO3-Angel-default.md';
    }
    ...
    getStatus() {
        return {
            ...
            currentSystemPromptFile: this.getCurrentSystemPromptFile(),
