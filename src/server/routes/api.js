...
// GET /api/system-prompts/list - List all available system prompts
router.get('/system-prompts/list', asyncHandler(async (req, res) => {
    try {
        const prompts = await getAvailableSystemPrompts();
        res.json({
            success: true,
            data: prompts
        });
...
// POST /api/system-prompts/select - Select a system prompt
router.post('/system-prompts/select', asyncHandler(async (req, res) => {
    ...
    try {
        const result = await selectSystemPrompt(filename);
        
        if (result.success) {
            res.json({
                success: true,
                data: {
                    message: `System prompt changed to: ${filename}`,
                    filename: filename,
                    timestamp: new Date().toISOString()
                }
            });
            ...
            res.status(404).json({
                success: false,
                message: result.message || `System prompt file '${filename}' not found`
            });
        }
    } catch (error) {
        res.status(500).json({
            success: false,
            message: 'Failed to select system prompt',
            error: error.message
        });
    }
}));

// GET /api/system-prompts/current - Get current system prompt info
router.get('/system-prompts/current', asyncHandler(async (req, res) => {
    try {
        const current = await getCurrentSystemPrompt();
        res.json({
            success: true,
            data: current
        });
        ...
        res.status(500).json({
            success: false,
            message: 'Failed to get current system prompt',
            error: error.message
        });
    }
}));
...
async function getAvailableSystemPrompts() {
    const systemPromptsDir = getSystemPromptsDirectory();
    
    if (!await fs.pathExists(systemPromptsDir)) {
        console.warn(`System prompts directory not found: ${systemPromptsDir}`);
        return [];
    }
    ...
    const mdFiles = files
        ...
        .map(file => ({
            ...
            description: getSystemPromptDescription(file),
            path: path.join(systemPromptsDir, file)
        }));
    
    console.log(`Found ${mdFiles.length} system prompt files in: ${systemPromptsDir}`);
    return mdFiles;
}

function getSystemPromptDescription(filename) {
    const descriptions = {
        'VEO3-Angel-default.md': 'Original VEO3 video generation assistant system prompt',
        'claude-creative.md': 'Creative writing and artistic content generation',
        'gpt-technical.md': 'Technical documentation and code generation',
        'gemini-analytical.md': 'Data analysis and research assistance',
        'general-assistant.md': 'General purpose AI assistant'
    };
    
    return descriptions[filename] || 'Custom system prompt';
}

function getSystemPromptsDirectory() {
    // Check if we're in a packaged Electron app
    const isPackaged = process.mainModule && process.mainModule.filename.indexOf('app.asar') !== -1;
    
    if (isPackaged) {
        // In packaged app: use resources/assets/system-prompts
        const appPath = path.dirname(process.execPath);
        return path.join(appPath, 'resources', 'assets', 'system-prompts');
    } else {
        // In development: system-prompts are under assets in the current working directory
        return path.join(process.cwd(), 'assets', 'system-prompts');
    }
}
...
async function selectSystemPrompt(filename) {
    ...
    if (!await fs.pathExists(filePath)) {
        return {
            success: false,
            message: `System prompt file '${filename}' not found`
        };
    }
    
    try {
        // Load the new system prompt
        const content = await fs.readFile(filePath, 'utf8');
        
        // Update the prompt processor to use the new system prompt
        ...
        console.error(`❌ Failed to load system prompt '${filename}':`, error);
        return {
            success: false,
            message: `Failed to load system prompt: ${error.message}`
        };
    }
}
...
async function getCurrentSystemPrompt() {
    try {
        const status = promptProcessor.getStatus();
        
        // Try to determine current system prompt filename
        let currentFilename = 'VEO3-Angel-default.md'; // default fallback
        
        // If we can get more detailed status, extract the filename
        if (status && status.systemPromptLoaded) {
            currentFilename = status.currentSystemPromptFile || currentFilename;
        }
        
        return {
            filename: currentFilename,
            name: currentFilename.replace('.md', '').replace(/-/g, ' ').replace(/\b\w/g, l => l.toUpperCase()),
            description: getSystemPromptDescription(currentFilename),
            loaded: status?.systemPromptLoaded || false,
            timestamp: new Date().toISOString()
        };
    } catch (error) {
